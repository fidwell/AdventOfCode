...
public class Puzzle11Solver : IPuzzleSolver
{
    ...
    private static string Solve(bool useSample, long gapSize)
    {
        ...
        var rowsWithNoGalaxies = data.RowsWhere(row => row.All(c => c != '#'));
        var galaxyPairs = ArrayExtensions.Pairs(galaxyCoordinates).ToArray();

        var distances = galaxyPairs.Select(pair =>
        {
